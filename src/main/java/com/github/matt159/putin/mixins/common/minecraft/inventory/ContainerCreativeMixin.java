...
public class ContainerCreativeMixin {
    ...
    private void addSlotsToContainer(EntityPlayer player, CallbackInfo ci) {
        ...
        for(i = 0; i < 5; ++i) {
            for(int j = 0; j < 18; ++j) {
                PutinUtil.addSlotToContainer(cc, new Slot(GuiContainerCreative.field_147060_v, i * 18 + j, 9 + j * 18, 18 + i * 18));
            }
        }
        ...
        int col;
        for (col = 0; col < 9; ++col) {
            PutinUtil.addSlotToContainer(cc, new Slot(inventoryPlayer, col, 9 + col * 18, 112));
        }

        //right half of hotbar
        for (col = 0; col < 9; ++col) {
            PutinUtil.addSlotToContainer(cc, new Slot(inventoryPlayer, col + 63, 9 + (col + 9) * 18, 112));
        }
